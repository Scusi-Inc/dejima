...
// host selects which authenticated gh host to read; empty/"github.com" uses the
// default host, anything else (a GitHub Enterprise host) is passed to gh via
// --hostname so an enterprise token can be seeded.
func LocalGitHubLogin(host string) (login, token string, err error) {
	...
	defer cancel()
	var hostArgs []string
	if h := strings.TrimSpace(host); h != "" && h != "github.com" {
		hostArgs = []string{"--hostname", h}
	}
	tokOut, err := exec.CommandContext(ctx, "gh", append([]string{"auth", "token"}, hostArgs...)...).Output()
	if err != nil {
		return "", "", fmt.Errorf("gh auth token: %w", ghExitErr(err))
	}
	loginOut, err := exec.CommandContext(ctx, "gh", append([]string{"api", "user", "--jq", ".login"}, hostArgs...)...).Output()
	if err != nil {
