...
// SpawnDir returns ~/.dejima/spawn — the daemon's store of operator-granted
// ephemeral-sub-agent spawn budgets (per island). Created 0700.
func SpawnDir() (string, error) {
	root, err := Root()
	if err != nil {
		return "", err
	}
	dir := filepath.Join(root, "spawn")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return "", err
	}
	return dir, nil
