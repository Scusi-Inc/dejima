...
- [x] Commit **authorship** matches the selected identity, not the host gitconfig. **A** (task #19): daemon materializes a per-island gitconfig (name=login, email=GitHub noreply) mounted at `/opt/host/gitconfig`; numeric id captured at `auth push` for the canonical `<id>+<login>@users.noreply` form, login-only fallback otherwise. Needs a re-`auth push` of existing identities to capture the id, and recreate of identity islands to pick up the gitconfig. **M** live verify pending.
