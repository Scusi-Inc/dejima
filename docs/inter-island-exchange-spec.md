...
- **Phase 1 — intra-island mailbox.** New `internal/mailbox` (or similar) + a typed
  message + notify; `dejima msg` CLI; ledgered. Lowest risk; lands first.
- **Phase 2 — inter-island info channel.** New `internal/link` broker modeled on
  `internal/capability`/`internal/port`; operator grants + scopes; A2A-shaped
  envelope; `link.*` ledger; API endpoints; `dejima link` CLI. **+ positioning.md edit.**
- **Phase 3 — inter-island action delegation.** The gate: per-action authz
  (pre-authorized action allowlist per channel) + a human-in-the-loop confirm hook;
  deny-all + fail-closed; every action ledgered.
