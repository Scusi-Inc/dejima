...
**Status:** Adopted — V1 built and **validated against a live Docker host** (`scripts/integration.sh`, green on Minion 2026-06-12). Phase 2 (Home Island) core built; brain-driven path and read-write deferred. Living record.
...
| Live-Docker end-to-end validation (`scripts/integration.sh`) | **Green** |
...
profile (which needs read-write and liveness). V1 is **built and validated** under exactly these
constraints:
...
`scripts/integration.sh` is the end-to-end dogfood against a **live Docker host** (green on
Minion, 2026-06-12). In a throwaway `$HOME` it starts its own `dejimad`, builds the island image,
and asserts, against real containers:
