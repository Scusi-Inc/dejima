...
- [~] **Multi-island sibling view in TUI / `dejima ls`** — **CLI shipped:** `dejima ls -g`/`--group` groups islands sharing a repo under a per-repo header (with an island count), so multi-agent projects read as one. Pure `groupByRepo` helper, unit-tested. **Remaining:** the equivalent grouping in the interactive TUI tree. (UI-only)
