...
- [ ] **macOS notarization** — sign + notarize + staple the darwin binaries with the Apple Developer ID (already held) so downloads aren't quarantined by Gatekeeper. The workflow has a TODO marking where the `macos-latest` signing step slots in; CI holds the signing secrets. *Do this before opening to others.* (half day + Apple setup)
...
- [ ] **Release consumers — Homebrew formula + client binary installer** — point the brew formula (and a client-only `curl | bash` binary installer) at the release tarballs with checksum verify; the release CI can auto-bump the tap formula's `sha256`. **Best wired against the first real v0.1.0 assets** (the `sha256`s don't exist until then). `install.sh` stays the *source* path for the full server (it needs the Dockerfile + service scripts). (half day, right after the first release)
