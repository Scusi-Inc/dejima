...
- [~] **macOS notarization** — sign + notarize the darwin binaries with the Apple Developer ID (already held) so downloads aren't Gatekeeper-quarantined. *Prep done:* `make release-binaries` codesigns darwin when `CODESIGN_IDENTITY` is set, and [`docs/release-notarization.md`](release-notarization.md) is a drop-in runbook (Apple cert + 6 Actions secrets + the macOS-runner workflow diff). **Remaining:** create the Developer ID cert + API key, add the secrets, apply the workflow diff. *Do this before opening to others.* (half day, mostly Apple-side)
